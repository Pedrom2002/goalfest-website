...
export async function POST(req: NextRequest) {
  ...
  if (companion_count === 1 && data.companion_nome && data.companion_email) {
    ...
    if (compInsertError) {
      if (compInsertError.code === "23505" && data.companion_email === data.email) {
        // Same email for main guest and companion — unique index blocks second insert.
        // Send a second email to the same address with the companion name + main token.
        try {
          await sendRsvpEmail({
            to: data.email,
            name: data.companion_nome,
          ...
          console.error("[rsvp] companion same-email send", e);
        }
      } else {
