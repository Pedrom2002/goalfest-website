        ...
        {doAnimate ? (
          <AnimatePresence mode="popLayout">
            <motion.div
              ...
              className="absolute inset-0 flex items-center justify-center bg-black/30 backdrop-blur-sm border border-white/20 rounded-lg font-display text-3xl md:text-5xl font-bold text-white"
            >
              {display}
            </motion.div>
          </AnimatePresence>
        ) : (
          <div className="absolute inset-0 flex items-center justify-center bg-black/30 backdrop-blur-sm border border-white/20 rounded-lg font-display text-3xl md:text-5xl font-bold text-white tabular-nums">
            {display}
          </div>
        )}
      </div>
...
export default function CountdownTimer() {
    ...
    <div className="flex items-center gap-4 md:gap-6">
      {['--', '--', '--', '--'].map((v, i) => (
        <div key={i} className="flex flex-col items-center gap-1">
          <div className="w-16 h-20 md:w-24 md:h-28 flex items-center justify-center bg-black/30 backdrop-blur-sm border border-white/20 rounded-lg font-display text-3xl md:text-5xl font-bold text-white">{v}</div>
        </div>
      ))}
    </div>
  )
  ...
  if (time === 'started') return (
    <div className="flex items-center justify-center px-6 py-3 bg-white/10 border border-white/40 rounded-full">
      <span className="w-2 h-2 rounded-full bg-white animate-pulse mr-3" />
      <span className="text-white font-bold uppercase tracking-widest text-sm">{t('countdown_live')}</span>
    </div>
  )
